...
class SpaceInvaders():
    ...
    def __init__(self, display: bool = False, High_score = 0):
        # player
        ...
        pygame.display.set_caption(
            "Welcome to Space Invaders Game by:YsN, Salma, Victor")
        ...
        self.playerImage = pygame.image.load(getURL('data/Ship.png'))
        self.reset()
        pygame.mixer.music.load('game/data/background.wav')
    ...
    def get_state(self):
        ...
        bulletState = int(self.get_bullet_state() == "rest")
    ...
    def reset(self):
        ...
        for _ in range(SpaceInvaders.NO_INVADERS):
            self.invaderImage.append(
                pygame.image.load(getURL('data/Target.png')))
    ...
    def step(self, action):
        ...
        if action == 1:  # GO RIGHT
            self.player_Xchange = 1.7
        if action == 2:  # FIRE
            self.fire = pygame.mixer.Sound("game/data/bullet.wav")
            # self.fire.play()
            self.player_Xchange = 0
        ...
        for i in range(SpaceInvaders.NO_INVADERS):
            ...
            if collision:
                self.explosion = pygame.mixer.Sound("game/data/explosion.wav")
                # self.explosion.play()
                reward = 1
                self.score_val += 1
                self.bullet_Y = 600
    ...
    def show_score(self, x, y):
        score = self.font.render(
            f"Points: {self.score_val}/{self.High_score}", True, (255, 255, 255))
